import { $invisible, $removeVisibility, $visibility, compilerAssert, getDiscriminatedUnion, getDiscriminator, getLifecycleVisibilityEnum, isVisible, navigateType, resetVisibilityModifiersForClass, setMediaTypeHint, } from "@typespec/compiler";
...
export function applyMergePatchTransform(ctx, input, nameTemplate, options) {
    let reported = false;
    navigateType(input, {
        intrinsic: (i) => {
    ...
    compilerAssert(type.kind === "Model", "Expected the root of the MergePatch transform to be a Model");
    return type;
}
